...
<%
	boolean formIsFilled = request.getParameter("formIsFilled") != null
			...
			if (formIsFilled && autoAdd) {
				...
				try {
					action.addVisit(newVisit, request.getParameter("patientID"),
							request.getParameter("scheduledDate"), request.getParameter("createdDate"),
							request.getParameter("weight"), request.getParameter("bloodPressure"),
							request.getParameter("FHR"), request.getParameter("numChildren"),
							request.getParameter("LLP"));
					loggingAction.logEvent(TransactionType.SCHEDULE_NEXT_OFFICE_VISIT, loggedInMID.longValue(),
							pid, Long.toString(newVisit.getID()));
...
<%
				...
				try {
					action.addVisit(newVisit, request.getParameter("patientID"),
							...
							request.getParameter("FHR"), request.getParameter("numChildren"),
							request.getParameter("LLP"));
					loggingAction.logEvent(TransactionType.CREATE_OBSTETRIC_OFFICE_VISIT,
							loggedInMID.longValue(), pid, Long.toString(newVisit.getID()));
					response.sendRedirect("viewObstetricsVisit.jsp");
				} catch (FormValidationException e) {
%>
<div align=center>
...
%>
<div align="center">
	<form action="addObstetricsVisit.jsp" method="post" id="addObVisitForm">
		<input type="hidden" name="formIsFilled" value="true"> <br />
		<div style="width: 50%; text-align: center;">Automatically Add
			New Obstetrics Office Visit</div>
		<br /> <input type="hidden" name="autoAdd" value="true"> <input
			type="hidden" name="patientID" value="<%=pid%>"> <input
			type="hidden" name="scheduledDate" value="<%=scheduledDate%>">
		<input type="hidden" name="weight" value="0"> <input
			type="hidden" name="bloodPressure" value="000/000"> <input
			type="hidden" name="FHR" value="0"> <input type="hidden"
			name="numChildren"
			value="<%=StringEscapeUtils.escapeHtml("" + (latestVisit.getNumChildren()))%>">
		<input type="hidden" name="LLP" value="<%=latestVisit.getLLP()%>">
		<input type="hidden" name="createdDate"
			value="<%=dateFormat.format(now)%>"> <input type="submit"
			style="font-size: 14pt; font-weight: bold;" value="Auto Add">
	</form>
</div>
<%
	} catch (Exception e) {
%>
<div align=center>
	<span class="iTrustError">No Previous Visits, Auto Add Not
		Available!</span>
</div>
<%
	}
...
<div align=center>
	...
	<form action="addObstetricsVisit.jsp" method="post" id="addObVisitForm">
		<input type="hidden" name="formIsFilled" value="true"> <br />
		<div style="width: 50%; text-align: center;">Please Enter New
			Obstetrics Office Visit</div>
		<br />
		<table class="fTable">
			<tr>
			...
			<tr>
				...
				<td><input type="text" name="scheduledDate" value=""> <input
					type=button value="Select Date"
					onclick="displayDatePicker('scheduledDate');"></td>
			</tr>
			<tr>
			...
			<tr>
				...
				<td><input type="hidden" name="createdDate"
					value="<%=dateFormat.format(now)%>"><%=dateFormat.format(now)%></td>
			</tr>

		</table>
		<br /> <input type="submit"
			style="font-size: 14pt; font-weight: bold;" value="Add Visit">
	</form>
	<br />
</div>
<%
...
<br />
<div align=center>
	<span class="iTrustMessage" id="addObVisitPatientError"> The
		patient is not an obstetrics patient. Please <a
		href="addObstetricsVisit.jsp">try again</a>.
	</span>
</div>
<br />
<%
...
<br />
<div align=center>
	<span class="iTrustMessage" id="addObVisitHCPError"> You are not
		an OB/GYN HCP. Please <a href="/iTrust/">go back</a> and create a
